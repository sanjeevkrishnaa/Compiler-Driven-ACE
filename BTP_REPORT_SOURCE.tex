...
% Keep the report as one continuous technical narrative. The existing
% chapter-level headings render as ordinary top-level sections without a
% chapter page break or a displayed "Chapter" label.
...
\begin{tikzpicture}[
  ...
  decision/.style={draw, diamond, aspect=2.1, align=center, inner sep=1.5pt,
                   fill=orange!12},
  outcome/.style={draw, rounded corners, align=center, minimum width=2.7cm,
                  minimum height=0.85cm, fill=green!8},
  arrow/.style={-{Stealth[length=2mm]}, thick}
]
\node[block] (trace) {Compiler trace and\\request identity};
\node[block, right=9mm of trace] (prepare) {Fixed, dynamic, or adaptive\\preparation decision};
\node[decision, right=10mm of prepare] (ready) {Exact target pair\\ready at release?};
\node[outcome, above right=8mm and 10mm of ready] (hit) {Consume exact pair\\pre-ready transfer};
\node[decision, below right=10mm and 10mm of ready] (slots) {All endpoint slots\\available together?};
\node[outcome, right=12mm of slots] (demand) {Demand generation\\and completion};
\node[outcome, below=8mm of slots] (wait) {Wait without partial\\endpoint allocation};
...
\end{tikzpicture}
\caption{Request-specific compiler preparation and demand recovery. The cap-four
wait-on-demand control enforces the atomic endpoint check shown in the lower
branch; historical ACE cap-three values retain their documented legacy RSVP
admission behavior.}
