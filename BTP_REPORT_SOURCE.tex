...
\begin{tikzpicture}[
  ...
  policy/.style={draw, rounded corners, align=center, minimum width=3.1cm,
                 minimum height=1.0cm, fill=orange!12},
  runtime/.style={draw, rounded corners, align=center, minimum width=3.2cm,
                  minimum height=1.0cm, fill=green!8},
  metric/.style={draw, rounded corners, align=center, minimum width=3.1cm,
                 minimum height=0.9cm, fill=gray!12},
  arrow/.style={-{Stealth[length=2mm]}, thick}
]
\node[input] (intent) {Compiler request intent\\endpoint $A \rightarrow B$};
\node[policy, right=9mm of intent] (offline) {Offline compiler route\\predict occupancy and select path};
\node[runtime, right=9mm of offline] (adaptive) {Adaptive compiler-driven route\\adapt to live congestion};
\node[metric, below=12mm of offline] (compare) {Compare XY, offline route,\\and adaptive route};
\node[input, below=12mm of intent] (state) {Runtime state\\memory, queues, failures, links};
\draw[arrow] (intent) -- (offline);
\draw[arrow] (offline) -- (adaptive);
\draw[arrow] (state) -- (adaptive);
\draw[arrow] (offline) -- (compare);
\draw[arrow] (adaptive) -- (compare);
\end{tikzpicture}
