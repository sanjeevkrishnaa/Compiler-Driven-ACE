...
def linear_entanglement_generation(verbose=False):
    ...
    modules = ['timeline', 'network_manager', 'resource_manager', 'rule_manager', 'generation', 'purification', 'swapping', 'bsm', 'node','optical_channel']
...
def app_20_node_bottleneck_request2_queue():
    ...
    num_nodes = len(name_to_apps)
    traffic_matrix = TrafficMatrix(num_nodes)
    traffic_matrix.bottleneck_20(1)
    request_queue = []
    traffic_matrix.get_request_queue_tts(request_queue=request_queue, request_period=1, delta=0.02, start_time=0, end_time=200, memo_size=1, fidelity=0.6, entanglement_number=1)
    for request in request_queue:
        id, src_name, dst_name, start_time, end_time, memo_size, fidelity, entanglement_number = request
        app = name_to_apps[src_name]
        app.start(dst_name, start_time, end_time, memo_size, fidelity, entanglement_number, id)
...
def mesh_request_queue():
    ...
    network_config = 'config/grid_4x4_ace_0.json'
    ...
    for router in network_topo.get_nodes_by_type(RouterNetTopo.QUANTUM_ROUTER):
        ...
        router.resource_manager.purify = False

    num_nodes = len(name_to_apps)
    # traffic_matrix = TrafficMatrix(num_nodes)
    # traffic_matrix.bottleneck_20(1)
    request_queue = []
    request_queue.append((0, 'router_0_0', 'router_0_3', 0, 1e11+5e10, 1, 0.01, 1))
    # request_queue.append((1, 'router_0_0', 'router_0_2', 1e11, 5e11, 2, 0.88, 1))
    # traffic_matrix.get_request_queue_tts(request_queue=request_queue, request_period=1, delta=0.02, start_time=0, end_time=200, memo_size=1, fidelity=0.6, entanglement_number=1)
    for request in request_queue:
        id, src_name, dst_name, start_time, end_time, memo_size, fidelity, entanglement_number = request
        app = name_to_apps[src_name]
    ...
    fidelity_dict      = defaultdict(list)
    for _, app in name_to_apps.items():
        time_to_serve_dict |= app.latency
        fidelity_dict |= app.entanglement_fidelities

    for reservation, time_to_serve in sorted(time_to_serve_dict.items()):
        fidelity = fidelity_dict[reservation][0]
        print(f'reservation={reservation}, time to serve={time_to_serve / MILLISECOND}, fidelity={fidelity:.6f}')
